...
.. code-block:: yaml
    ...
    In this example, the filter will be applied only to observations taken between 09:00:00 and 18:00:00, between 1st January and 25th May of every year.  Note that datetime components are not yet 'loop aware'.  That is, a where clause between May and February for example would require two min/max ranges, not the one:  one covering the Jan-Feb period end a second to cover the May-Dec period.
